...
def huffman_encode(s):  # функция кодирования строки в коды Хаффмана
    ...
    for ch, freq in Counter(s).items():  # построим очередь с помощью цикла, добавив счетчик, уникальный для всех
        # листьев
...
def main():
    ...
    print(f'Количество уникальных символов - {len(code)}\nдлина закодированной строки - {len(encoded)}')  # выведем число символов и длину закодированной строки
